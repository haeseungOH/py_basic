...
ydata = np.random.randint(5, size = (1000,1))   # 범주 5개, 시험과목 0:국어 ~ 4:체육 라고 가정
print(xdata[:2])
print(ydata[:2])    # [[2][0]] 학습시label은 one-hot 처리 후 학습에 참여
...
print(model.summary())
history = model.fit(xdata, ydata, epochs=1000, batch_size=32, verbose=2, shuffle=True)   # ydata=one-hotdata
