...
try:
    ...
    df1 = pd.DataFrame(cursor.fetchall(), columns=['사번','이름','부서명','연봉','직급'])
    print(df1)
...
# - 부서명별 연봉의 합, 연봉의 최대/최소값을 출력
    ...
    print(df)
    print('-----'*10)
    jik_ypay = df.groupby(['부서'])['연봉'].sum()
    print(jik_ypay)
    print('-----'*10)
    jik_ypay1 = df.groupby(['부서'])['연봉'].max()
    print(jik_ypay1)
    print('-----'*10)
    jik_ypay2 = df.groupby(['부서'])['연봉'].min()
    print(jik_ypay2)
...
# - 부서명별 연봉의 평균으로 가로 막대 그래프를 작성
    jik_ypay3 = df.groupby(['부서'])['연봉'].mean()
    
    print(jik_ypay3, type(jik_ypay3)) # Series
    print(jik_ypay3.index)
    print(jik_ypay3.values)
    
    plt.rcParams['axes.unicode_minus'] = False
    plt.barh(range(len(jik_ypay3)), jik_ypay3, alpha = 0.5)
    plt.xlabel('연봉')
