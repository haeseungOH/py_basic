...
# k 값 ? 방법1 ) 엘보우(elbow) 기법 - 클러스터 간 SSE의 차이를 이용해 최적의 클러스터 수를 제시

def elbow_func(x):
    sse = []
    for i in range(1, 11):   # k값을 1~10까지 추천
...
# k 값 ? 방법2 ) 실루엣(silhouette) 기법 - 클러스터의 품질을 정량적으로 계산해 표현
'''
실루엣(silhouette) 기법
