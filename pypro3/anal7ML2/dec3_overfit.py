# sklearn 모듈(라이브러리)이 지원하는 과적합 방지 함수(method)의 이해
# iris dataset
...
dt_clf.fit(train_data, train_label) # train_data, train_label 학습시키기
...
for train_index, test_index in kfold.split(features):
    ...
    # 반복할 때마다 정확도 측정 
    acc = np.round(accuracy_score(ytest, pred), 3)     #소수 3째자리 까지 출력
...
# 위와 같은 불균형 데이터인 경우는 KFold 보다는 StratifiedKFold를 사용한다. 
...
# 여러 개의 속성 값 중 max_depth, min_samples_split 에 대해서 최적의 값 찾기 
