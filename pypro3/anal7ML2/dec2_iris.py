...
# 다항분류 : 출력시 softmax 함수를 사용 - 결과가 확률값으로 출력되며 가장 큰 값의 인덱스를 분류 범주값으로 적용
...
y = iris.target
print(x[:3])
print(y[:3], set(y))    # {0, 1, 2} 꽃의 종류 3가지
...
# C 속성: L2 규제(모델에 패널티 적용)   값이 작을수록 규제가 더 강한 정규화 규제가 진행됨
...
dot_data = StringIO()
tree.export_graphviz(read_model, feature_names = iris.feature_names[2:4], out_file = dot_data, filled = True, rounded = True)
graph = pydotplus.graph_from_dot_data(dot_data.getvalue())
graph.write_png('tree_iris.png') # 그래프를 이미지로 저장
